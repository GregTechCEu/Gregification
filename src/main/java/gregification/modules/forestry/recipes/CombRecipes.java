...
import gregification.modules.forestry.ForestryConfig;
import gregification.modules.forestry.ForestryModule;
import gregification.modules.forestry.ForestryUtils;
import gregification.modules.forestry.bees.GTCombType;
...
public class CombRecipes {
    ...
    public static void initGTCombs() {
        ...
        if (Loader.isModLoaded(ModIDs.MODID_EB)) {
            addCentrifugeToItemStack(GTCombType.ENERGY, new ItemStack[]{/*ForestryCommonProxy.gtDrops.get(GTDropType.HOT_COOLANT),*/ MetaItems.ENERGIUM_DUST.getStackForm(), ModuleCore.getItems().refractoryWax.getItemStack()}, new int[]{/*20*100,*/ 20 * 100, 50 * 100}, Voltage.HV, 196);
            ItemStack wax = ModuleCore.getItems().beeswax.getItemStack();
            if (Loader.isModLoaded(ModIDs.MODID_MB)) {
                wax = BaseUtility.getModItem(ModIDs.MODID_MB, "wax", 2);
            }
            addCentrifugeToItemStack(GTCombType.LAPOTRON, new ItemStack[]{/*ForestryCommonProxy.gtDrops.get(GTDropType.LAPIS),*/ OreDictUnifier.get(OrePrefix.dust, Materials.Lapotron), wax}, new int[]{/*20*100,*/ 15 * 100, 40 * 100}, Voltage.HV, 196);
        }
        //addCentrifugeToItemStack(GTCombType.PYROTHEUM, ...);
        //addCentrifugeToItemStack(GTCombType.CRYOTHEUM, ...);
        ...
        if (ForestryConfig.nerfGTCombs) {
            ...
            //addCentrifugeToMaterial(GTCombType.FIRESTONE, ...);
    ...
    private static void addCentrifugeToItemStack(GTCombType comb, ItemStack[] item, int[] chance, Voltage volt, int duration) {
        ItemStack combStack = ForestryUtils.getCombStack(comb);
